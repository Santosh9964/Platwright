...
test.only('@WebBrowser context-validation Error login',async ({browser})=>
{
  // chrome-plugins/cookies
  const context = await browser.newContext();
  const page = await context.newPage();

// The below line explain  how to block the network calls, here we blocking the css call
  page.route('**/*.css',route => route.abort());

  // The below line explain  how to block the network calls, here we blocking the image calls
  ...
  // The below line explain how to track events like request--> it displays all the request made
  // on is the listener, it listens when even request happens simillarly response happens
